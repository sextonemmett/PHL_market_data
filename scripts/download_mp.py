...
BASE_DOWNLOAD_PAGE = "https://www.iemop.ph/market-data/rtd-market-clearing-price/"
DEFAULT_MD_FILE_PREFIX = "/var/www/html/wp-content/uploads/downloads/data/MP/MP_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download daily RTD market clearing price CSVs from IEMOP, "
            "validate them, and concatenate the range into one combined Parquet file."
        )
    )
...
def download_and_check(
    ...
    destination = raw_dir / f"MP_{file_date.strftime('%Y%m%d')}.csv"
...
def main() -> int:
    ...
    manifest_name = f"mp_qc_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv"
    ...
    combined_name = f"MP_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.parquet"
